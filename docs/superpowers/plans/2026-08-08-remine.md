...
# this package directory — on sys.path so `import remine.*` resolves. The
# repo has no pyproject/installed package, and the documented CLI is the
# script form, so without this the documented command fails outright.
