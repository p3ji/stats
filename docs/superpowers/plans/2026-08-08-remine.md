...
def rank_order(df, dim, cfg):
    ...
    for period, chunk in df.groupby("REF_DATE"):
        ...
        f = Fact(
            probe="rank_order", cut={dim: f"{top[dim]} leads, {bottom[dim]} trails"},
...
def rank_reversal(df, dim, cfg):
    ...
    for prev, cur in zip(ordered, ordered[1:]):
        ...
        f = Fact(
            probe="rank_reversal",
            cut={dim: f"{cur.meta['leader']} overtook {prev.meta['leader']}"},
...
def gap_trend(df, dim, cfg):
    ...
    for pair, series in by_pair.items():
        ...
        hi, lo = pair.split("|")
        f = Fact(
            probe="gap_trend", cut={dim: f"{hi} vs {lo}"}, values=[change],
...
def check_bare_numerals(text: str) -> list[str]:
    ...
    for match in NUMERAL.finditer(stripped):
        ...
        if YEAR.match(raw) or ORDINAL.match(raw + tail.strip()[:2]):
            continue
        bad.append(raw)
    return bad
